...
void OptimizationBase<Base>::Loop(int Njob, int ijob){
  ...
  for(int i = 0; i < m_N_Cut; i++){
    char svar[100];
    sprintf(svar,"var%d",i);
    //string svar = "var"+to_string(i);
    m_Tree->Branch(svar,&(m_CutVal[i]));
  }
  double Nbkg;
...
void OptimizationBase<Base>::AddBackground(string filename){
  ...
  while(link){
    ...
    if(name.find("TPrime") != string::npos){ //STOP
...
template class OptimizationBase<ReducedBase>;
