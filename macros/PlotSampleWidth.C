...
void PlotSampleWidth(){
  ...
  for(int f = 0; f < Nfile; f++){
    ...
    for(int h = 0; h < Nhist; h++){
      ...
      for(int s = 0; s < 3; s++){
	interval = GetInterval(vMT[h], 0.5*pow(2, s));
  ...
  TLegendEntry* le;
  le = (TLegendEntry*) leg2->AddEntry(gr[0][0], "0.5 #sigma", "p");
  le->SetMarkerColor(kBlack);
  le = (TLegendEntry*) leg2->AddEntry(gr[0][1], "1.0 #sigma", "p");
  le->SetMarkerColor(kBlack);
  le = (TLegendEntry*) leg2->AddEntry(gr[0][2], "2.0 #sigma", "p");
  le->SetMarkerColor(kBlack);
  leg2->SetLineColor(kWhite);
  leg2->SetFillColor(kWhite);
  leg2->SetShadowColor(kWhite);
